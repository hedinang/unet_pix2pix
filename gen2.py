...
for f in os.listdir(root):
    if '.png' in f:
        ...
        h, w = img.shape[:2]
        txt = open('{}/{}.txt'.format(root, f.split('.')[0]))
        ...
        for line in lines[:-1]:
            ...
            if cls != 5:
                ...
                for i in range(100):
                    ...
                    cv2.imwrite('data/input/{}_{}'.format(f, angle), input)
                    cv2.imwrite('data/unet/{}_{}'.format(f, angle), unet)
                    cv2.imwrite('data/pix2pix/{}_{}'.format(f, angle), pix2pix)
